...
def run_variant_sno4(X_feat, U, Y_log, folds, out_dir, variant, genes, coords, regions,
    ...
    for fold, (tr, va) in enumerate(folds, 1):
        ...
        folds_mses.append(mses); folds_pcc.append(pccs)
    save_metrics(out_dir, variant, genes, folds_mses, folds_pcc, preds, Y_log, regions, coords)
# === PATCH: multi-slide ingestion & per-slide feature extraction ===
import glob

def load_all_slides_and_build_features(root_dir: str,
                                       slides_pattern: str,
                                       args):
    """
    Returns concatenated arrays across slides, with per-spot slide ids:
      X_base_all, U_all, lam_all, Y_log_all, coords_all, slide_id_all (int array), X_aug_all (after all enabled feature groups)
    Notes:
      - Graph/Laplacian are computed PER SLIDE (not across slides).
      - Moran/Geary (AC) are computed PER SLIDE then broadcast, then concatenated.
    """
    slide_dirs = sorted([p for p in glob.glob(os.path.join(root_dir, slides_pattern)) if os.path.isdir(p)])
    if len(slide_dirs) == 0:
        raise RuntimeError(f"No slides found under: {os.path.join(root_dir, slides_pattern)}")

    # If genes not provided, compute pooled HVGs ONCE (pooled variance across slides)
    pooled_var = None
    pooled_var_names = None
    if not args.gene_list:
        for sd in slide_dirs:
            adata, _, _ = load_10x_visium(sd)
            ...
            if pooled_var is None:
                pooled_var = var
                pooled_var_names = np.array(adata.var_names)
            else:
                # align by gene name
                # make a quick map for current var names
                cur_names = np.array(adata.var_names)
                name_to_idx = {n:i for i,n in enumerate(cur_names)}
                aligned = np.zeros_like(pooled_var)
                for i, n in enumerate(pooled_var_names):
                    j = name_to_idx.get(n, -1)
                    aligned[i] = var[j] if j>=0 else 0.0
                pooled_var += aligned
        idx = np.argsort(pooled_var)[::-1][:args.genes]
        fixed_genes = [str(g) for g in pooled_var_names[idx]]
    else:
        fixed_genes = [g.strip() for g in args.gene_list.split(",") if g.strip()]
    ...
    coords_all = []
    slide_id_all = []
    X_aug_all = []  # will be built per slide with same enabled stacks
    sid_counter = 0

    for sd in slide_dirs:
        adata, img, coords = load_10x_visium(sd)
        adata = adata[(adata.obs["in_tissue"].astype(int)==1), :].copy()
        coords = coords.astype(float)


        # Targets for this slide: lock to fixed_genes (drop missing)
        genes = [g for g in fixed_genes if g in adata.var_names] if fixed_genes else []
        if not genes:
            raise RuntimeError(f"No overlap between fixed gene list and slide var_names in {sd}")
        subX = adata[:, genes].X
        Y = subX.toarray().astype(np.float32) if sparse.issparse(subX) else np.asarray(subX, np.float32)
        Y_log = np.log1p(np.maximum(Y, 0.0)).astype(np.float32)

        # Base features
        X_base = spot_patch_features(img, coords, patch_px=args.patch_px)

        # Graph + spectrum (PER SLIDE)
        ...
        # ---- feature groups (reuse your existing code paths) ----
        ...
        U_all.append(U)
        lam_all.append(lam)
        Y_log_all.append(Y_log)
        coords_all.append(coords)
        X_aug_all.append(X_aug)
        slide_id_all.append(np.full((N_spots,), sid_counter, dtype=np.int32))
        sid_counter += 1

    # Concatenate across slides
    X_base_all = np.vstack(X_base_all).astype(np.float32)
    U_all = np.vstack(U_all).astype(np.float32)
    Y_log_all = np.vstack(Y_log_all).astype(np.float32)
    coords_all = np.vstack(coords_all).astype(np.float32)
    X_aug_all = np.vstack(X_aug_all).astype(np.float32)
    slide_id_all = np.concatenate(slide_id_all, axis=0)
    return X_base_all, U_all, Y_log_all, coords_all, slide_id_all, X_aug_all, fixed_genes
...
def main():
    ...
    # === PATCH: new CLI flags ===
    ap.add_argument("--cv_mode", choices=["spot_kfold", "group_kfold", "loso"], default="spot_kfold",
                help="Cross‑val mode: spot-wise (baseline), GroupKFold by slide, or strict LOSO by slide.")
    ap.add_argument("--slides_pattern", type=str, default="GSM*",
                help="Glob under dataset_dir for slide folders when running multi-slide.")
    ap.add_argument("--multi_slide", type=int, default=1,
                help="If 1, load all slides under dataset_dir/slides_pattern. If 0, use --sample_id single slide.")
    ...
    # === Multi-slide vs single-slide ===
    if int(args.multi_slide) == 1:
        log.info("Load ALL slides and build per-slide features…")
        (X_base, U, Y_log, coords, slide_id, X_aug, genes) = load_all_slides_and_build_features(
            root_dir=args.dataset_dir,
            slides_pattern=args.slides_pattern,
            args=args
        )
    # No per-slide AnnData to write here; just dump coords for reference
        pd.DataFrame(coords, columns=["x_px","y_px"]).to_csv(os.path.join(args.out_dir, "coords_px.csv"), index=False)

    # Provide a dummy monotone spectrum for abl_spectral_pool (bands only depend on order)
        lam = np.linspace(0.0, 1.0, U.shape[1], dtype=np.float32)

        log.info(f"[multi] X_base={X_base.shape}, X_aug={X_aug.shape}, U={U.shape}, Y_log={Y_log.shape}, coords={coords.shape}, slides={int(slide_id.max())+1}")
    else:
    ...
    # [keep your existing single-slide gene selection → Y, Y_log]
    # [keep your existing single-slide X_base / U / lam / feature-block construction → X_aug]
    # (No changes to your single-slide block except we define slide_id below)

    # define slide_id for single-slide mode so CV code works uniformly
        slide_id = np.zeros((N_spots,), dtype=np.int32)


    # ==== SINGLE‑SLIDE FEATURE BUILD (only when --multi_slide=0) ====
    if int(args.multi_slide) == 0:
...
# ==== END SINGLE‑SLIDE FEATURE BUILD ====


    # 5-fold CV over spots (single-slide dataset)
    # === PATCH: pick folds based on cv_mode ===
    from sklearn.model_selection import KFold, GroupKFold
    indices = np.arange(Y_log.shape[0])
    if args.cv_mode == "spot_kfold":
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        folds = list(kf.split(indices))
    elif args.cv_mode == "group_kfold":
        gkf = GroupKFold(n_splits=5)
        folds = list(gkf.split(indices, groups=slide_id))
    else:  # "loso"
        uniq = np.unique(slide_id)
        folds = []
        for s in uniq:
            va = np.where(slide_id == s)[0]
            tr = np.where(slide_id != s)[0]
            folds.append((tr, va))
